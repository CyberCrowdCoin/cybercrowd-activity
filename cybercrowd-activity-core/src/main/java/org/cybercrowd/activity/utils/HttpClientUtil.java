package org.cybercrowd.activity.utils;
...
public class HttpClientUtil {
	 ...
	 * @return 响应文本
	 */
	public static String sendHttp(org.cybercrowd.activity.utils.HttpRequestMethod.HttpRequestMethodEnum requestMethod, String url, Map<String, Object> params,
	...
	public static String postWithParamsForString(String url, Map<String,String> paramsMap,Map<String,String> headerMap){
		...
		logger.info("url:{} params:{} header:{}" ,url,JSON.toJSONString(paramsMap),JSON.toJSONString(paramsMap));
		try {
			...
			if(statusCode == 200){
				...
				HttpEntity entity = response.getEntity();
				logger.info("失败结果:{}",EntityUtils.toString(entity));
			}
		} catch (IOException e) {
			e.printStackTrace();
			logger.info("Http请求异常:{}",e.getMessage());
	...
	public static String postJson(String url, String str, Map<String,String> headers) {
		...
		try {
			...
			logger.info("Cost:{},results={}", end - start,e.getMessage());
			throw new RuntimeException(e);
		} finally {
			long end = System.currentTimeMillis();
			logger.info("Cost:{}, results={}", end - start,ret.toString());
			return ret.toString();
		}
	}
}
